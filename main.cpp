...
void create_fbo()
{
	...
	glBindRenderbufferEXT(GL_RENDERBUFFER_EXT, rb[1]);
	glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_RGBA8, WIDTH, HEIGHT);
	glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_RENDERBUFFER_EXT, rb[1]);
...
void init(const char* in, char c)
{
	...
	glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, ls[0].s_res, ls[0].t_res, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
	glBindTexture(GL_TEXTURE_2D, 0);
...
int main( int argc, char** argv )
{
	...
	string filepath = "C:\\HomeWork&Project\\CS684\\aperture\\Alice\\Alice\\";
	string data = "Alice";
